...
def main(q_init, v_peak_full, t_peak, t_total, dt):
    ##############################################
    p_0 = np.array([[q_init[0]], [q_init[1]], [q_init[2]]])
    v_0 = np.zeros((3, 1))
    a_0 = np.zeros((3, 1))
    v_peak = np.array([[v_peak_full[0]], [v_peak_full[1]], [v_peak_full[2]]])


    obj_traj_1 = generate_spline_by_peak_speed(p_0, v_0, a_0, v_peak, t_peak, t_total, dt)
    T_1, traj_1 = obj_traj_1.do_calculation()
    ###################################################
    p_0 = np.array([[q_init[3]], [q_init[4]], [q_init[5]]])
    v_0 = np.zeros((3, 1))
    a_0 = np.zeros((3, 1))
    v_peak = np.array([[v_peak_full[3]], [v_peak_full[4]], [v_peak_full[5]]])

    obj_traj_2 = generate_spline_by_peak_speed(p_0, v_0, a_0, v_peak, t_peak, t_total, dt)
    T_2, traj_2 = obj_traj_2.do_calculation()
    ###################################################
    p_0 = np.array([[q_init[6]], [0.0], [0.0]])
    v_0 = np.zeros((3, 1))
    a_0 = np.zeros((3, 1))
    v_peak = np.array([[v_peak_full[6]], [0.0], [0.0]])

    obj_traj_3 = generate_spline_by_peak_speed(p_0, v_0, a_0, v_peak, t_peak, t_total, dt)
    T_3, traj_3 = obj_traj_3.do_calculation()
    ###################################################

    positions = np.concatenate((traj_1[0:3,:], traj_2[0:3,:], np.reshape(traj_3[0,:], (-1, traj_3[0,:].shape[0]))), axis=0)
    # print(positions)
    velocities = np.concatenate((traj_1[3:6,:], traj_2[3:6,:], np.reshape(traj_3[3,:], (-1, traj_3[3,:].shape[0]))), axis=0)
    # print(velocities)
    accelerations = np.concatenate((traj_1[6:9,:], traj_2[6:9,:], np.reshape(traj_3[6,:], (-1, traj_3[6,:].shape[0]))), axis=0)

    # print(positions.shape[1])

    traj_full = {"time_traj": T_1, "positions": positions, "velocities": velocities, "accelerations": accelerations}

    # print(traj_full["time_traj"])
...
if __name__ == '__main__':
    ...
    q_init = np.zeros(7)
    v_peak_full = [0.0, 0.0, 0.0, 0.0, radians(2.0), 0.0, radians(2.0)]
    t_peak = 5.0
    t_total = 10.0
    dt = 0.001

    traj_full = main(q_init, v_peak_full, t_peak, t_total, dt)
    # print(traj_full)
